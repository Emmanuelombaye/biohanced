...
function StepCard({ step }: { step: (typeof STEPS)[number] }) {
  return (
    <article className="rounded-[16px] border border-bio-neutral-200 bg-bio-white p-6 transition-shadow hover:shadow-[0_12px_32px_rgba(10,11,14,0.06)]">
  ...
  return (
    <BiohancedSection tone="cream">
      <div className="bio-container">
        ...
        <BiohancedMobileRail className="mt-12 md:hidden">
          {STEPS.map((step) => (
        ...
        {/* Desktop — full grid, no clipped cards */}
        <div className="mt-12 hidden min-w-0 gap-6 md:grid md:grid-cols-2 lg:grid-cols-4">
          {STEPS.map((step) => (
